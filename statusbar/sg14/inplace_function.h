...
struct vtable
{
    ...
    constexpr explicit vtable() noexcept
        : invoke_ptr([](storage_ptr_t, Args&&...) -> R { SG14_INPLACE_FUNCTION_THROW(std::bad_function_call()); })
    ...
    constexpr explicit vtable(wrapper<C>) noexcept
        ...
        , copy_ptr([](storage_ptr_t dst_ptr, storage_ptr_t src_ptr) { ::new (dst_ptr) C{(*static_cast<C*>(src_ptr))}; })
...
template <class F, class... Args>
struct is_invocable_r_impl<decltype(std::declval<F>()(std::declval<Args>()...), void()), void const, F, Args...> : std::true_type
{};
...
class inplace_function<R(Args...), Capacity, Alignment>
{
  ...
  public:
    ...
    {
        static_assert(
            inplace_function_detail::is_valid_inplace_dst<Capacity, Alignment, Cap, Align>::value, "conversion not allowed");
    }
  ...
  private:
    ...
    inplace_function(
        vtable_ptr_t vtable_ptr, typename vtable_t::process_ptr_t process_ptr, typename vtable_t::storage_ptr_t storage_ptr)
