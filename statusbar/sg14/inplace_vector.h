...
struct ipvbase_assignable<T, Copyable, false>
{
    ...
    ipvbase_assignable(ipvbase_assignable const&) = delete;
    void operator=(ipvbase_assignable&&) = delete;
    void operator=(ipvbase_assignable const&) = delete;
...
struct SG14_INPLACE_VECTOR_TRIVIALLY_RELOCATABLE_IF(std::is_trivially_relocatable_v<T>) ipvbase
{
    ...
    constexpr T const* base_data_() const { return data_; }
    constexpr void set_size_(size_t n) { size_ = n; }

    constexpr explicit ipvbase() noexcept {}
    ipvbase(ipvbase const& rhs) noexcept(std::is_nothrow_copy_constructible_v<T>)
    {
        if constexpr (std::is_trivially_copy_constructible_v<T>) {
            std::memmove((void*)this, (void const*)std::addressof(rhs), sizeof(ipvbase));
        } else {
            std::uninitialized_copy_n(rhs.data_, rhs.size_, data_);
            size_ = rhs.size_;
        }
    }
    ipvbase(ipvbase&& rhs) noexcept(
    ...
    void operator=(ipvbase const& rhs) noexcept(std::is_nothrow_copy_constructible_v<T> && std::is_nothrow_copy_assignable_v<T>)
    {
        if constexpr (
    ...
    ipvbase(ipvbase const&)
        requires std::is_trivially_copy_constructible_v<T>
    = default;
    ipvbase(ipvbase&&)
        requires std::is_trivially_move_constructible_v<T>
    = default;
    ipvbase& operator=(ipvbase const&)
...
struct ipvbase_zero
{
    ...
    constexpr T const* base_data_() const { return nullptr; }
...
using ipvbase_t = std::conditional_t<
    N == 0,
    ipvbase_zero<T>,
    std::conditional_t<std::is_trivially_copyable_v<T>, ipvbase_trivial<T, N>, ipvbase<T, N>>>;

template <class T, size_t N>
class inplace_vector
    : ipvbase_assignable<T>
    , ipvbase_t<T, N>
{
  ...
  public:
    ...
    inplace_vector& operator=(inplace_vector const&) = default;
    ...
    constexpr explicit inplace_vector(size_t n, value_type const& value) { assign(n, value); }

    template <
        class It,
        std::enable_if_t<std::is_base_of_v<std::input_iterator_tag, typename std::iterator_traits<It>::iterator_category>, int> = 0>
    ...
    constexpr void assign(size_t n, value_type const& value)
    {
        if (n <= size_) {
    ...
    constexpr void resize(size_type n, value_type const& value)
    {
        if (n < size_) {
            std::destroy(data() + n, data() + size_);
            set_size_(n);
    ...
    constexpr T const* data() const noexcept { return this->base_data_(); }
    ...
    value_type& unchecked_push_back(value_type const& value) { return unchecked_emplace_back(value); }
    ...
    constexpr value_type* try_push_back(value_type const& value) { return try_emplace_back(value); }
    ...
    value_type& push_back(value_type const& value) { return emplace_back(value); }
    ...
    iterator insert(const_iterator pos, value_type const& value) { return emplace(pos, value); }
...
#    endif
    ...
    constexpr void swap(inplace_vector& b) noexcept(
    ...
    friend constexpr void swap(inplace_vector& a, inplace_vector& b) noexcept(noexcept(a.swap(b))) { a.swap(b); }

    constexpr friend bool operator==(inplace_vector const& a, inplace_vector const& b)
    {
        if (a.size_ != b.size_) {
            return false;
        }
        T const* adata = a.data();
        T const* bdata = b.data();
    ...
    constexpr friend auto operator<=>(inplace_vector const& a, inplace_vector const& b)
    {
        T const* adata = a.data();
        T const* bdata = b.data();
    ...
    constexpr friend bool operator>(inplace_vector const& a, inplace_vector const& b) { return (b < a); }
    constexpr friend bool operator<=(inplace_vector const& a, inplace_vector const& b) { return !(b < a); }
    constexpr friend bool operator>=(inplace_vector const& a, inplace_vector const& b) { return !(a < b); }
    constexpr friend bool operator!=(inplace_vector const& a, inplace_vector const& b) { return !(a == b); }
#endif
};
